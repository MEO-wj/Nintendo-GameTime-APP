...
export function createCatalogRouter(deps: AppDependencies): Router {
  ...
  router.get("/api/catalog/games/:externalId/prices", requireAuth, async (ctx) => {
    const parsed = detailParamsSchema.safeParse(ctx.params);
    if (!parsed.success) {
      ctx.status = 400;
      ctx.body = { message: "Invalid params", issues: parsed.error.flatten() };
    ...
    const catalogGame = await deps.catalogService.getCatalogGame(parsed.data.externalId);
    if (!catalogGame) {
      ctx.status = 404;
      ctx.body = { message: "Catalog game not found" };
      return;
    ...
    const priceSet = await deps.eshopPriceService.getRegionalPrices(
      parsed.data.externalId,
      catalogGame.title,
      catalogGame.priceAmount
    );
    ctx.body = priceSet;
  });
